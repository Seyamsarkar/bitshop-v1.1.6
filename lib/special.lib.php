...
function count_cat_files($cat_id, $downloads) {
  $cat_num = 0;
  foreach($downloads as $value) {
	if ($value['FileCat'] == $cat_id) {
	  $cat_num++;
	}
  }
  return $cat_num;
...
function get_key_data($dir, $code) {
  $key_data = file_get_contents($dir.$code);
  if ($key_data !== false) {
	return $key_data;
  } else {
...
function alt_btc_pair($btc_val, $api_url) {
  ...
  if (!empty($alt_parr) && bccomp($alt_parr->price, '0') == 1) {
	return bcdiv($btc_val, $alt_parr->price);
  } else {
	return false;
  }
}
...
function item_box_html($item, $admin=false) {
  global $exch_orig;
  global $exch_rate;
  global $dec_shift;
  global $curr_symbol;
