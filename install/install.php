...
<?php
  ...
  if (isset($_POST['pub_key'])) {
    $new_config = array(
	  'pub_rsa_key' => $_POST['pub_key'], 'sec_str' => rand_str()
    );
...
<div style="width:600px;margin-left:auto;margin-right:auto;">
  ...
  <p>In order to make the ticker update periodically with the correct exchange rate you will need to set up a cron job. If you have cPanel you can easily set up cron jobs. The cron job time fields are fairly self-explanatory and easy to understand, they dictate how often the script will be called, which determines how often the ticker will update. The command part is where you need to input the following line of text:</p>

  <p><i>php -q /home/serverpath/cron/tick_update.php</i></p>

  <p>All you need to do is change the first part "/home/serverpath/" to the location of your BitShop installation, then submit the cron job and your ticker should update however often you specified (anything less than an hour may put unnecessary strain on the ticker API). If you wish you can also create a second cron job which points to the clean_trans.php script, it will delete old unconfirmed transactions every time the script runs.</p>
