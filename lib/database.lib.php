...
function best_file() {
  ...
  if (!empty($result) && $result != 'N/A') {
	while ($row = mysqli_fetch_assoc($result)) {
	  $row_num = $row['FileSales'];
	  if ($highest_num < $row_num) {
	    $highest_num = $row_num;
		$highest_row = $row;
	  }
	}
...
function top_file() {
  ...
  if (!empty($result) && $result != 'N/A') {
	while ($row = mysqli_fetch_assoc($result)) {
	  if ($row['FileVoteNum'] > 0) {
	    $row_num = get_rating($row);
	  } else {
	    $row_num = 0;
	  }
	  if ($highest_num < $row_num) {
...
function best_files($num_files) {
  $num_files = (int) $num_files;
  $highest_nums = array_fill(0, $num_files, -1);
  $highest_rows = array_fill(0, $num_files, null);
  $result = select_from_where('Products', '*', "FileActive = 1");
  if (!empty($result) && $result != 'N/A') {
	while ($row = mysqli_fetch_assoc($result)) {
	  $row_num = $row['FileSales'];
	  for ($i=0;$i<$num_files;$i++) {
	    if ($highest_nums[$i] < $row_num) {
		  for ($x=($num_files-1);$x>$i;$x--) {
	        $highest_nums[$x] = $highest_nums[$x-1];
		    $highest_rows[$x] = $highest_rows[$x-1];
		  }
		  $highest_nums[$i] = $row_num;
		  $highest_rows[$i] = $row;
		  break;
	    }
	  }
...
function count_file_cat($cat_id) {
  ...
  "FileCat = $cat_id AND FileActive = 1"), 0);
}
...
function count_file_cats($cat_ids) {
  ...
  foreach ($cat_ids as $key => $value) {
    $cat_count += count_file_cat($value);
  }
  return $cat_count;
}
...
function list_files($cat_id, $start, $sort_meth='FileID', 
  ...
  if ($multi_cat == true) {
	...
	$id_first = (int) $cat_ids[0];
	$ids_exp = "FileCat = $id_first";
    for ($i=1;;$i++) {
	  if (isset($cat_ids[$i])) {
        $id_val = (int) $cat_ids[$i];
	    $ids_exp .= " OR FileCat = $id_val";
	  } else {
	    break;
	  }
    }
    return select_from_where('Products', '*', "($ids_exp) AND FileActive = 1", 
	"ORDER BY $sort_meth $sort_order LIMIT $start, $item_num");
  } else {
    $cat_id = (int) $cat_id;
    return select_from_where('Products', '*', "FileCat = $cat_id AND FileActive = 1", 
	"ORDER BY $sort_meth $sort_order LIMIT $start, $item_num");
  }
}

function create_file($type, $stock, $name, $desc, $cat, $code, $price, $method) {
  ...
  return insert_into('Products', 'FileType, FileStock, FileName, FileDesc, '.
                                 'FileCat, FileCode, FilePrice, FileMethod',
					             "'$type', $stock, '$name', '$desc', $cat, ".
								 "'$code', $price, '$method'");
}
...
function search_files($search_phrase, $start, $item_num, $allitems=0) {
  ...
  if ($like_qrs != '()') {
	...
	if ($result['count'] > 1) {
	  ...
	  $rholder[0] = mysqli_fetch_assoc($result['query']);
	  $result['query'] = $rholder;
	} else { $result['query'] = 0; }
    return $result;
  } else {
    $result['count'] = 0;
...
function similar_files($search_phrase, $item_id, $item_cat, $item_price) {
  $item_id = (int) $item_id;
  ...
  if (!empty($search_phrase)) {
	...
	"(FileCat = $item_cat AND ABS(FilePrice - $item_price) < 1))", "LIMIT 0, 9");
  } else {
    return 0;
  }
}
...
function order_num($days, $confirmed=true) {
  $days = (int) $days;
  $conf_str = ($confirmed) ? "(Status = 'Confirmed') AND" : "(Status != 'Confirmed') AND";
  return mysqli_result(select_from_where('Orders', 'COUNT(*)', 
  "$conf_str Created BETWEEN UTC_TIMESTAMP() - INTERVAL $days DAY AND UTC_TIMESTAMP()"), 0);
}

function total_income($days) {
  $days = (int) $days;
  return mysqli_result(select_from_where('Orders', 'SUM(Amount)', 
  "(Status = 'Confirmed') AND Created BETWEEN UTC_TIMESTAMP() ".
  "- INTERVAL $days DAY AND UTC_TIMESTAMP()"), 0);
}

function average_income($days) {
  $days = (int) $days;
  return mysqli_result(select_from_where('Orders', 'AVG(Amount)', 
  "(Status = 'Confirmed') AND Created BETWEEN UTC_TIMESTAMP() ".
  "- INTERVAL $days DAY AND UTC_TIMESTAMP()"), 0);
}
...
function get_pcats() {
  return select_from_where('Categories', '*', "Parent = 0", "ORDER BY CatPos ASC"); 
}

function get_scats($parent) {
  $parent = (int) $parent;
  return select_from_where('Categories', '*', "Parent = $parent", "ORDER BY CatPos ASC"); 
}
...
function update_cat($cat_id, $cat_pos, $parent, $name, $image) {
  $cat_pos = (int) $cat_pos;
  $parent = (int) $parent;
  $cat_id = (int) $cat_id;
  return multi_update('Categories', "CatPos = $cat_pos, Parent = $parent, ".
                      "Name = '$name', Image = '$image'", "CatID = $cat_id");
}
